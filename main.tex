...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
An object's moment of inertia is analogous to its mass in the context of rotation, but, it depends on the distance from the center of rotation.\\
For \textbf{point particles} of mass $m_i$, the moment of inertia is given by
$$
I=\sum_i m_i r_i^2
$$

We can generalize to \textbf{extended objects} with arbitrary mass distributions by integrating:
$$
I=\int r^2 d m
$$
...
%------------ Summary rotation vs. translation Content ---------------
\begin{tikzpicture}
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
\begin{tabular}{ll}
\multicolumn{2}{c}{Pure \textbf{Translation} (Fixed Direction)} \\
\hline Position & \(x\) \\
...
\begin{tabular}{ll}
\multicolumn{2}{c}{Pure \textbf{Rotation} (Fixed Axis)} \\
\hline Angular position & \(\theta\) \\
Angular velocity & \(\omega=\frac{d \theta}{d t}\) \\
Angular acceleration & \(\alpha=\frac{d \omega}{d t}\) \\
Rotational inertia & \(I\) \\
Newton's second law & \(\tau_{\text{net}}=I \alpha\) \\
Work & \(W=\int \tau d \theta\) \\
Kinetic energy & \(K=\frac{1}{2} I \omega^2\) \\
Power (constant torque) & \(P=\tau \omega\) \\
Work-kinetic energy theorem & \(W=\Delta K\) \\
\end{tabular}

    \end{minipage}
};
%------------ Summary rotation vs. translation Header ---------------------
\node[fancytitle, right=10pt] at (box.north west) {Rotational vs. translational};
\end{tikzpicture}
